...
class SurvivorBuddy:
    ...
    neck_pitch_min = 35
    neck_pitch_max = 150
    # bigger = more forwards
    neck_yaw_min = 20
    neck_yaw_max = 160
    # smaller = MY left, survivor buddy's right
    head_roll_min = 0
    head_roll_max = 180
    # bigger = counterclockwise from MY persepctive
    head_pitch_min = 30
    head_pitch_max = 120
    # bigger= down
        ...
        if port_address != None:
            ...
            if is_windows_os:
                ...
                usb_serial_paths = [
                    ...
                    if file_name.startswith("tty.usbserial-")
                ]
                if len(usb_serial_paths) == 0:
                    raise Exception(
                        f"""[SurvivorBuddy Controller] I don't see any available USB connections (e.g. nothing matches /dev/tty.usbserial-XXXX)"""
                    )
                if len(usb_serial_paths) == 1:
                    ...
                    if mac_default_address in usb_serial_paths:
                        ...
                        print(
                            "(just add SurvivorBuddy(mac_default_address=/dev/somethin'))"
                        )
            ...
            while self.still_running:
                ...
                if len(self.scheduled_actions) > 1:
                    ...
                    self.connection.write(
                        bytes(
                            f"""{neck_pitch}{neck_yaw}{head_roll}{head_pitch}\n""",
                            "utf-8",
                        )
                    )
    ...
    def set_joints(self, neck_pitch, neck_yaw, head_roll, head_pitch, speed=40):
        ...
        assert (
            speed <= 100 and speed >= 0.1
        ), "Speed of an action must be in the range 0.1 to 100"
        assert (
            neck_pitch >= SurvivorBuddy.neck_pitch_min
            and neck_pitch <= SurvivorBuddy.neck_pitch_max
        )
        assert (
            neck_yaw >= SurvivorBuddy.neck_yaw_min
            and neck_yaw <= SurvivorBuddy.neck_yaw_max
        )
        assert (
            head_roll >= SurvivorBuddy.head_roll_min
            and head_roll <= SurvivorBuddy.head_roll_max
        )
        assert (
            head_pitch >= SurvivorBuddy.head_pitch_min
            and head_pitch <= SurvivorBuddy.head_pitch_max
        )
        ...
        for _ in range(int(max(abs(each) for each in diffs))):
            ...
            new_neck_pitch = _increment_joint_value(
                neck_pitch_diff, new_neck_pitch, neck_pitch
            )
            new_neck_yaw = _increment_joint_value(neck_yaw_diff, new_neck_yaw, neck_yaw)
            new_head_roll = _increment_joint_value(
                head_roll_diff, new_head_roll, head_roll
            )
            new_head_pitch = _increment_joint_value(
                head_pitch_diff, new_head_pitch, head_pitch
            )
